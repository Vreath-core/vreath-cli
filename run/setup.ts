...
(async()=>{
    try{
        ...
        const encrypted_pri = CryptoJS.AES.encrypt(private_key,my_key).toString(CryptoJS.enc.Utf8);
        await promisify(fs.writeFile)('./keys/private/'+my_key+'.txt',encrypted_pri,'utf-8');
        await promisify(fs.writeFile)('./keys/public/'+my_key+'.txt',public_key,'utf-8');
    }
    catch(e){
        console.log(e);
    }
